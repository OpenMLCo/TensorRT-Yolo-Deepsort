...
class PreprocessYOLO(object):
    ...
    def _load_and_resize(self, ori_image):
        ...
        ori_image = Image.fromarray(ori_image) #TODO 修正为fromarray 否则无法从摄像头获取img
        ...
        image_resized = ori_image.resize(
            new_resolution, resample=Image.BICUBIC)
        image_resized = np.array(image_resized, dtype=np.float32, order='C')
        return ori_image, image_resized
...
class PostprocessYOLO(object):
    ...
    def process(self, outputs, resolution_raw):
        ...
        """
        # print('outputs.shape', len(outputs)) #3
        ...
        # boxes, categories, confidences = self.process_yolo_output(
        #     outputs, resolution_raw)
    ...
    def _reshape_output(self, output):
        ...
        return np.reshape(output, (dim1, dim2, dim3, dim4))  #(h, w, 3, 85) numpy array
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        # print('nparray  boxes : ', boxes.shape)
        # print(boxes)
        # print('nparray  categories : ', categories.shape)
        # print('nparray confidence   : ', confidence.shape)


        #boxs.shape = (1, 4), categories = (1, ), confidences = (2,)
        return boxes, categories, confidences
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        """
        grid_h, grid_w, _, _ = output_reshaped.shape #grid_h be like 13, 26, 52...
        ...
        # box_xy += (box_wh / 2.) #TODO 注释掉中心点位置才正确
        boxes = np.concatenate((box_xy, box_wh), axis=-1)

        # boxes: centroids, box_confidence: confidence level, box_class_probs:
        # class confidence
        #boxes : (13, 13, 3, 4) 一个点三个框， 坐标四个，
        #box_confidence = (13, 13, 3, 80)
